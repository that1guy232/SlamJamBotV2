 ...
 */
    static void sendEmbedWithReactions(IChannel channel, EmbedObject embed, List<String> emojiAlias) {
        RequestBuffer.request(() -> {
            try{
                Long messageID = channel.sendMessage(embed).getLongID();
                for (int i = 0; i < emojiAlias.size(); i++) {
                    channel.getMessageByID(messageID).addReaction(EmojiManager.getForAlias(emojiAlias.get(0)));
                }
